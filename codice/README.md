...
3. Copy the `.mp4` file **together with its `.THM` file** (same name,
   created automatically since v1.0.2) into `Memory Stick/VIDEO/`.
4. Find the film under Video → Memory Stick.

### PSP says "Corrupted data" / "Incompatible data"?

Since v1.0.2 this should no longer happen for files produced by the app:
every download now gets a short ASCII-safe filename (no spaces, accents
or special characters — the most common cause of that error), an
automatic `.THM` cover, and a post-conversion compatibility check
(MSNV container, H.264 Baseline ≤480x272, AAC 48 kHz, printed in the log
as `[PSP-Check]`). If the PSP still refuses a file, check that:

1. You copied **both** the `.mp4` and the `.THM` into `ms0:/VIDEO/`
   (all caps folder name, in the root of the Memory Stick — **not** into
   `MP_ROOT/100MNV01`, which needs special `M4Vxxxxx.MP4` names and is
   only for very old firmware).
2. You are on a recent firmware / custom firmware (the `VIDEO` folder
   needs PSP firmware ≥ 3.30).
...
next to every converted video. The included **PSP Thumbnail Maker**
(`psp_thumbnail_maker.py` / `PSPThumbnailMaker.exe`) remains available if
you want to **replace** the automatic cover with a different frame or a
custom image.
...
3. Copia il file `.mp4` **insieme al suo file `.THM`** (stesso nome,
   creato in automatico dalla v1.0.2) in `Memory Stick/VIDEO/`.
4. Troverai il film nel menu Video → Memory Stick.

### La PSP dice "Dati danneggiati / incompatibili"?

Dalla v1.0.2 non dovrebbe più succedere per i file prodotti dall'app:
ogni download ora ha un nome corto e sicuro (solo ASCII, senza spazi,
accenti o caratteri speciali — la causa più comune di quell'errore), una
copertina `.THM` automatica e un controllo di compatibilità dopo la
conversione (contenitore MSNV, H.264 Baseline ≤480x272, AAC 48 kHz,
visibile nel log come `[PSP-Check]`). Se la PSP rifiuta comunque un file,
verifica che:

1. Hai copiato **sia** il `.mp4` **sia** il `.THM` in `ms0:/VIDEO/`
   (cartella in maiuscolo nella root della Memory Stick — **non** in
   `MP_ROOT/100MNV01`, che richiede nomi speciali tipo `M4Vxxxxx.MP4`
   e serve solo ai firmware vecchissimi).
2. Hai un firmware recente / custom firmware (la cartella `VIDEO`
...
Dalla v1.0.2 l'app principale **crea già da sola** il file `.THM`
accanto a ogni video convertito. Lo strumento incluso
**PSP Thumbnail Maker** (`psp_thumbnail_maker.py` /
`PSPThumbnailMaker.exe`) resta disponibile se vuoi **sostituire** la
copertina automatica con un altro fotogramma o un'immagine tua.
